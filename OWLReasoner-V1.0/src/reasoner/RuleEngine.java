...
public class RuleEngine {
	...
	private void processToDoList() {
		...
		while (!todo.isEmpty()) {
			...
			if (entry != null && !entry.getNode().isReset()) {
				...
				if (currNode == null)
					currNode = n;
				// System.err.println("n "+ n.getId() +" entry "+ entry.getClassExpression() +"
				// curr node "+ currNode.getId());
	...
	//	}
		...
		if (sol.isSolved()) {
			...
			for(Node to : oldNodes) {
				...
				if (e != null)
					applyTransitiveRule(n, to, e.getLabel(), ds);
			}
	...
	private boolean addLabel(Node from, Node to, Set<OWLClassExpression> fillers, DependencySet ds, boolean newNode,
		...
		for (OWLClassExpression ce : fillers) {
			// System.err.println("add label : "+to.getId() +" "+ ce);
			...
			if (isConceptExist(to, ce)) {
				...
				if (ce instanceof OWLObjectOneOf) {
					 ...
					 * if(!djGrp.isEmpty()) { DependencySet clashSet = getClashSet(to, djGrp);
					 * if(!clashSet.isEmpty()) { if(!clashHandler(clashSet)) isInconsistent(to); }
					 * else isInconsistent(to); return; }
					 */
					...
					this.cg.addConceptToNode(to, cnds);
					if (!checkClash(to, ce)) {
						if (needReset((OWLObjectSomeValuesFrom)ce, to)) {
							reset(to);
							addToDoEntries(to);
						} else {
							addToDoEntry(to, ce, cnds);
	...
	private boolean canMerge(Node from, Node to) {
		...
		/*if(config.isSHOIQ() && to.isNominalNode()) {
			...
			maxCards.addAll(from.getnLabel().getCndList().getCdSet().stream()
					.filter(cnd -> cnd.getCe() instanceof OWLObjectMaxCardinality).collect(Collectors.toSet()));
			maxCards.addAll(to.getnLabel().getCndList().getCdSet().stream()
					.filter(cnd -> cnd.getCe() instanceof OWLObjectMaxCardinality).collect(Collectors.toSet()));
	...
	private boolean checkAtMost(Node from, Node to, OWLObjectMaxCardinality mc, DependencySet ds) {
			...
			for (Edge e : from.getOutgoingEdges()) {
				if (!e.isReset() && (e.isPredEdge() || e.getToNode().isNominalNode()) && e.getLabel().contains(role) && e.getToNode().getLabel().contains(filler)) {
					Node n = e.getToNode();
					//// System.err.println("to "+ to.getId());
			...
			//	System.out.println("outgoing edge:node " + n.getId() + "filler "+ filler);
				...
				if (isConceptExist(n, filler)) {
					...
					/*if (!((filler instanceof OWLClass) || (filler instanceof OWLObjectOneOf)
						...
						ConceptNDepSet cnd = n.getnLabel().getCndList().getCdSet().stream()
								.filter(cnds1 -> cnds1 != null && cnds1.getCe().equals(filler)).iterator().next();
						if (cnd != null)
	...
	private void checkExistingEntries(Node n, OWLClassExpression filler) {
		...
		//	System.err.println("ce "+ ce);
			if(ce instanceof OWLObjectMaxCardinality) {
				Set<OWLObjectPropertyExpression> s2 = new HashSet<>(this.getAllRoles(((OWLObjectCardinalityRestriction)ce).getProperty()));
				s2.retainAll(s1);
				if(!s2.isEmpty()) {
				...
				Set<OWLObjectPropertyExpression> s2 = new HashSet<>(this.getAllRoles(((OWLObjectAllValuesFrom)ce).getProperty()));
				s2.retainAll(s1);
				if(!s2.isEmpty()) {
	...
	public boolean applyOrRule(Node n, OWLObjectUnionOf objUn, ConceptNDepSet cn) {
		...
		// System.out.println("Applying or Rule..."+ n.getId() + objUn + " ds
		// "+ds.getMax());
	...
	private Node addConcept(Node n, OWLClassExpression ce, DependencySet ds) {
	...
	private boolean clashHandler(DependencySet clashSet, Node node) {
		...
		if (!clashSet.isEmpty()) {
			...
			///
				...
				System.err.println(
						"mergeBranching n: " + branches.get(level).mergeFrom.getId() + " node: " + node.getId());
	...
	// --- L(<x,y>) = {R} , exist_R.D -> C ~~>> exist_R.Top -> (C or forAll_R.neg_D)
	private boolean absorbRoleExtendedRule(Set<OWLObjectPropertyExpression> roles, Node x, DependencySet ds) {
		Set<OWLClassExpression> domain = new HashSet<OWLClassExpression>();
		for(OWLObjectPropertyExpression r : roles) {
			domain.addAll(intl.getExtendedDomainRestrictions(r));
		}
...
private AddConceptResult tryAddConcept(Node n,  OWLClassExpression ce) {
	...
	if (n.getLabel().stream().anyMatch(ce -> ce instanceof OWLObjectMinCardinality)) {
		for (OWLObjectMinCardinality minCard : n.getLabel().stream()
				.filter(ce -> ce instanceof OWLObjectMinCardinality).map(ce -> (OWLObjectMinCardinality) ce)
				.collect(Collectors.toSet())) {
			int card = minCard.getCardinality();
			int totalNodes = 0;
			OWLObjectPropertyExpression role = minCard.getProperty();
			OWLClassExpression filler = minCard.getFiller();
			for (Edge edge : n.getOutgoingEdges().stream().filter(e -> !e.isReset() && e.getLabel().contains(role))
			...
			if (totalNodes < card) {
				DependencySet ds = DependencySet.create(n.getnLabel().getCndList().getCdSet().stream()
						.filter(ce -> ce.getCe().equals(minCard)).iterator().next().getDs());
				if (!ds.isEmpty()) {
					ds.setCe(minCard);
					if (!clashHandler(ds, n))
						isInconsistent(n);
