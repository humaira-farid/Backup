...
import reasoner.RuleEngine.BranchHandler;
...
public class RuleEngine {
	...
	Map<Integer, BranchHandler> ILPBranches = new HashMap<Integer, BranchHandler>();
	...
	public RuleEngine(Internalization i, ToDoList todo, OWLDataFactory df, Configuration config, Logger LOG) {
		...
		//ILPBranchingPoint = INITBRANCHINGLEVELVALUE;
			...
			if (entry != null && !entry.getNode().isReset()) {
				...
				//	proceedWithProcessedEntries();
					...
					if (cg.getNodeBase().contains(entry.getNode())) {
						//System.out.println("n "+ n.getId() +" process entry here... "+ entry.getClassExpression());
	...
	private boolean processEntry(ToDoEntry entry) {
		// System.out.println(currNode.getId() +" entry "+ entry.getType());
	...
	public boolean callILP(DependencySet clashSet, Node n, Set<ToDoEntry> entries, DependencySet newDs, Set<OWLSubClassOfAxiom> learnedDisjointness, BranchHandler bhILP, OWLObjectPropertyExpression role) {
		//System.out.println("ilpPro.getInsideBranches().size "+ ilpPro.getInsideILPDisjunctions().size());
		save(n);
		if(role == null) {
		ILPPreprocessor ilpPro = bhILP.ilpPro;
		...
		if (sol.isSolved()) {
			Node nn = processILPResult(n, sol, entries, newDs);
			if(nn == null) {
			...
			System.out.println("here " + clashSet.getbpList());
			...
			ILPSolution sol = null;
			Set<ILPSolution>  solutions = new HashSet<>();
			Set<ILPPreprocessor>  ilpPros = new HashSet<>();
			Map<OWLObjectPropertyExpression, ILPPreprocessor> ilpProsMap = bhILP.ilpPros;
			Map<Integer, BranchHandler> insideILPBranchHandlers = new HashMap<>();
			for(OWLObjectPropertyExpression r : ilpProsMap.keySet()) {
				ILPPreprocessor ilpPro = ilpProsMap.get(r);
				insideILPBranchHandlers.putAll(ilpPro.getInsideILPBranchHandlers());
				
			}
			addInsideILPBranches(n, insideILPBranchHandlers);
			for(OWLObjectPropertyExpression r : ilpProsMap.keySet()) {
				ILPPreprocessor ilpPro = ilpProsMap.get(r);
				if(r.equals(role)) {
					ilpPro.addLearnedDisjointness(learnedDisjointness, n);
				...
				if(s.isSolved()) {
					...
					for(ILPPreprocessor ip : ilpPros) {
						Set<Integer> insideBranchLevels = ip.getInsideBranches(); 
						for(Integer i : insideBranchLevels){
							branches.remove(i);
						}
					}
					boolean handleClash = false;
					DependencySet clashSet1 = DependencySet.create();
					...
					for (ToDoEntry entry : entries) {
						if (!entry.getDs().isEmpty()) {
							...
							clashSet1.add(entry.getDs());
						}
					}
					if (handleClash) {
						if (!clashHandler(clashSet1, n)) {
						//	System.out.println("here ");
							isInconsistent(n);
						}
					} else
			...
			sol = combineSolutions(solutions); 
			Node nn = processILPResult(n, sol, entries, newDs);
			if(nn == null) {
				for(ILPPreprocessor ip : ilpPros) {
					Set<Integer> insideBranchLevels = ip.getInsideBranches(); 
	...
	private void addInsideILPBranches(Node n, Map<Integer, BranchHandler> insideILPBranchHandlers) {
	//	System.out.println("addInsideILPBranches...insideILPBranchHandlers.size()  " + insideILPBranchHandlers.size());
	//	save(n);
		//Map<Integer, BranchHandler> sorted =
		//		insideILPBranchHandlers.entrySet().stream().sorted(Map.Entry.comparingByKey()).collect(Collectors.toMap(keyMapper, valueMapper));
		List<Integer> bpList = new ArrayList<Integer>(insideILPBranchHandlers.keySet());
				Collections.sort(bpList);
		for(Integer bp : bpList) {
			this.branches.put(bp, insideILPBranchHandlers.get(bp));
			save(n);
			this.setCurLevel(bp);
			System.err.println("addInsideILPBranches...bp " + bp);
		}
		this.incCurLevel();
	...
	//			.forEach(en -> System.out.println(en.getDs().getbpList() + " entry: " + en.getClassExpression()));
		...
		if (!config.hasRoleHierarchy() && axiomRoles.size() > 1) {
			SetMultimap<OWLObjectPropertyExpression, ToDoEntry> roleNEntries = HashMultimap.create();
			for (ToDoEntry en : entries) {
				OWLClassExpression ce = en.getClassExpression();
				if (ce instanceof OWLObjectMinCardinality) {
					OWLObjectPropertyExpression r = ((OWLObjectMinCardinality) ce).getProperty();
					roleNEntries.put(r, en);
				} else if (ce instanceof OWLObjectMaxCardinality) {
					OWLObjectPropertyExpression r = ((OWLObjectMaxCardinality) ce).getProperty();
					roleNEntries.put(r, en);
				} else if (ce instanceof OWLObjectSomeValuesFrom) {
					OWLObjectPropertyExpression r = ((OWLObjectSomeValuesFrom) ce).getProperty();
					roleNEntries.put(r, en);
				} else if (ce instanceof OWLObjectHasValue) {
					OWLObjectPropertyExpression r = ((OWLObjectHasValue) ce).getProperty();
					roleNEntries.put(r, en);
				} else if (ce instanceof OWLObjectAllValuesFrom) {
					OWLObjectPropertyExpression r = ((OWLObjectAllValuesFrom) ce).getProperty();
					roleNEntries.put(r, en);
				}
			}
			Node nn = null;
			for (OWLObjectPropertyExpression r : roleNEntries.keySet()) {
				Set<Edge> relatedOutgoingEdges = new HashSet<>();
						outgoingEdges.stream().filter(e -> e.getLabel().contains(r)).forEach(e -> relatedOutgoingEdges.add(e));
				nn = callILP2(n, roleNEntries.get(r), subsumption, relatedOutgoingEdges);
				System.err.println("currNode " + currNode.getId());
				if (nn == null) {
					return nn;
				}
		 ...
		 return callILP2(n, entries, subsumption, outgoingEdges);
	}*/
	public Node callILP(Node n, Set<ToDoEntry> entries, Set<OWLSubClassOfAxiom> subsumption,
			Set<Edge> outgoingEdges) {
		...
		if (!config.hasRoleHierarchy() && axiomRoles.size() > 1) {
			...
			for (OWLObjectPropertyExpression r : roleNEntries.keySet()) {
				Set<Edge> relatedOutgoingEdges = new HashSet<>();
				outgoingEdges.stream().filter(e -> e.getLabel().contains(r)).forEach(e -> relatedOutgoingEdges.add(e));
				ILPPreprocessor ilpPro = new ILPPreprocessor(this, this.cg, roleNEntries.get(r), this.intl, this.df, n, relatedOutgoingEdges,subsumption, new HashMap<>(), r);
				ilpPros.add(ilpPro);
				bh.addIlpPro(r, ilpPro);
				ILPSolution s = null;
				try {
					s = ilpPro.callILP();
				} catch (IloException e) {
		...
		//	System.err.println("solutions size " + solutions.size());
			sol = combineSolutions(solutions);
		...
		//System.err.println(" outgoingEdges.size() "+outgoingEdges.size());
		...
		ILPPreprocessor ilpPro = new ILPPreprocessor(this, this.cg, entries, this.intl, this.df, n, outgoingEdges,subsumption, superRolesMap, null);
		bh.setIlpPro(ilpPro);
	...
	private ILPSolution combineSolutions(Set<ILPSolution> solutions) {
		ILPSolution finalSolution = new ILPSolution();
		for(ILPSolution sol : solutions) {
			for (EdgeInformation ei : sol.getEdgeInformation()) {
				finalSolution.addEdgeInformation(ei);
			}
		}
		if(finalSolution.getEdgeInformation().size()==0)
			finalSolution.setSolved(false);
	//	System.err.println("combined solution size " + finalSolution.getEdgeInformation().size());
		return finalSolution;
	}

	public Node processILPResult(Node n, ILPSolution sol, Set<ToDoEntry> entries, DependencySet newDs) {
		Set<Node> oldNodes = new HashSet<>();
			Set<Node> newNodes = new HashSet<>();
			DependencySet ds = DependencySet.create();
		//	System.err.println("sol.getEdgeInformation() size " + sol.getEdgeInformation().size());
			for (EdgeInformation ei : sol.getEdgeInformation()) {
				Set<Integer> insideBranchLevels =ei.getInsideILPBranchLevels(); 
				...
				//System.err.println("ds ilp : "+ds.getbpList() + " cnds.size " + ei.getCnds().size());
				...
				if (nodeIds.isEmpty()) {
					...
					if (ei.getFillers().stream().anyMatch(ce -> ce instanceof OWLObjectOneOf)) {
						 ...
						 */
						e = this.cg.addEdge(n, to, getAllRoles(roles), ds);
					//	System.err.println(n.getOutgoingEdges().size() + " edge added "+ e.getLabel());
						Node nn = this.absorbRoleRule(getAllRoles(roles), n, to, ds);
						if (nn == null || nn.getId() == -1)
							return nn;
						// e = this.cg.addEdge(n, to, roles, ds);
		...
		//	System.err.println("here outside");
	...
	private Set<Edge> checkOutGoingEdges(Node n, Set<ToDoEntry> entries2) {
		...
		for (ToDoEntry en : entries2) {
			if (en.getType().equals(NodeTag.GE)) {
				...
				OWLObjectAllValuesFrom av = (OWLObjectAllValuesFrom) en.getClassExpression();
				OWLObjectPropertyExpression role = av.getProperty();
				// System.out.println("entry for all "+n.getOutgoingEdges().size());
				for (Edge e : n.getOutgoingEdges()) {
				//	System.out.println("entry for all "+e.getLabel());
					if (cg.getNodeBase().contains(e.getToNode()) && e.getLabel().contains(role) && !e.isReset()
							&& !e.getToNode().isReset())
						outEdges.add(e);
				}
	...
	private void splitNode2(Node x) {
		...
		for (int i = 0; i < x.getCardinality() - 1; i++) {
			...
			newNode.getSimpleLabel().addAll(x.getSimpleLabel());
			newNode.getSimpleILPLabel().addAll(x.getSimpleILPLabel());
			newNode.getILPBranchingLevels().addAll(x.getILPBranchingLevels());
			newNode.getInsideILPDisjunct().putAll(x.getInsideILPDisjunct());
	...
	private void reset(Node n) {
		...
		for (Edge e : n.getOutgoingEdges()) {
			...
			if (e.isSuccEdge() && !e.isReset()) {
				...
				Node to = e.getToNode();
				if (!to.isNominalNode()) {
					System.err.println("reset to node " + to.getId() +" cg.getEdge(to, n) " + cg.getEdge(to, n));
					to.setReset(true);
	...
	private boolean addLabel1(Node from, Node to, Set<OWLClassExpression> fillers, DependencySet ds, boolean newNode,
			Set<ToDoEntry> entries) {
		if (newNode)
			return addLabelNewNode1(from, to, fillers, ds, entries);
		else
			return addLabelOldNode1(from, to, fillers, ds, entries);
	...
	private Node addLabel(Node from, Node to, Set<OWLClassExpression> fillers, Set<ConceptNDepSet> cnds, DependencySet nds, DependencySet newDs, boolean newNode,
		...
		if (newNode)
			...
			return addLabelOldNode(from, to, fillers,  cnds, newDs, entries, insideBranchLevels);
	...
	private Node addLabelOldNode(Node from, Node to, Set<OWLClassExpression> fillers, Set<ConceptNDepSet> cnds, DependencySet newDs, Set<ToDoEntry> entries, Set<Integer> insideBranchLevels) {
		...
		for (ConceptNDepSet cnd : cnds) {
			...
			DependencySet ds = DependencySet.plus(DependencySet.create(cnd.getDs()), DependencySet.create(newDs));
			for(Integer i : ds.getbpList()){
				if(insideBranchLevels.contains(i)) {
					this.branches.get(i).setDisjunctTaken(ce);
					this.branches.get(i).setNode(to);
	...
	private Node addLabelNewNode(Node from, Node to, Set<OWLClassExpression> fillers, Set<ConceptNDepSet> cnds, DependencySet nds, DependencySet newDs, Set<ToDoEntry> entries, Set<Integer> insideBranchLevels) {
		...
		for (ConceptNDepSet cnd : cnds) {
			...
			//	System.out.println("add label : "+ ce +" ds  :  "+ ds.getbpList());
				for(Integer i : ds.getbpList()){
					if(insideBranchLevels.contains(i)) {
						this.branches.get(i).setDisjunctTaken(ce);
						this.branches.get(i).setNode(to);
	...
	private boolean addLabelOldNode1(Node from, Node to, Set<OWLClassExpression> fillers, DependencySet ds, Set<ToDoEntry> entries) {
		//System.out.println("addLabelOldNode, node: "+ to.getId());
		addBackPropagatedConcepts(to, entries, fillers);
	...
	private boolean addLabelNewNode1(Node from, Node to, Set<OWLClassExpression> fillers, DependencySet ds, Set<ToDoEntry> entries) {
		System.out.println("addLabelNewNode, node: "+ to.getId());
		addTGAxiom(to, ds);
		for (OWLClassExpression ce : fillers) {
			if (!isConceptExist(to, ce)) {
	...
	public boolean applyForAllRule(Node from, OWLObjectAllValuesFrom objAV, DependencySet ds) {
		...
		System.out.println("Applying for all Rule..." + from.getNodeId() + ds.getbpList());
		/*
...
/*	private boolean getConsistencyCheckGE() {
	...
	public boolean applyOrRule(Node n, OWLObjectUnionOf objUn, ConceptNDepSet cn) {
		DependencySet ds = cn.getDs();
		 System.out.println("Applying or Rule..."+ n.getId() + objUn + " ds "+ds.getbpList());
	...
	private Node addConcept(Node n, OWLClassExpression ce, DependencySet ds, boolean isILPLabel) {
		...
		//System.err.println("Node " +n.getId()+" addConcept : " + ce +" ds "+ ds.getbpList());
		...
		//	System.err.println("exists : " + ce);
			...
			if (ce instanceof OWLObjectOneOf) {
				...
				if (needToAdd((OWLObjectCardinalityRestriction) ce, n)) {
					...
					//	System.out.println(" QCRClashSet "+ clashSet.getbpList());
						if (!clashSet.isEmpty()) {
							clashSet.setCe(ce);
							clashSet.setCeNNF(ce.getComplementNNF());
							if (!clashHandler(clashSet, n))
			...
			if (nom == null) {
				...
				if (nom.equals(n)) {
					...
					//	System.out.println("after Merge! " + to);
						...
						DependencySet clashSet = DependencySet.create(ds);
						if (!clashSet.isEmpty()) {
						//	clashSet.setCe(ce);
						//	clashSet.setCeNNF(ce.getComplementNNF());
							if (!clashHandler(clashSet, n))
	...
	private boolean clashHandler(DependencySet clashSet, Node node) {
		...
		//	 System.err.println(clashSet.getbpList());
			...
			if (branches.get(level) != null) {
				if (branches.get(level).isInsideILP()) {
					System.err.println("inside ILPBranching");
					OWLObjectPropertyExpression role = branches.get(level).role;
					...
					do {
						...
						if (branches.get(level) == null || branches.get(level).getNode() == null) {
						...
						if (branches.get(level) != null && branches.get(level).isInsideILP()) {
							...
							if(role == null) {
								...
								if(branches.get(level).role.equals(role)) {
									insideILPBranches.put(level, branches.get(level));
									clashSet.removeLevel(level);
									level = clashSet.getMax();
								}
					...
					return handleClashInsideILPBranching(clashSet, node, insideILPBranches, role);
				}

				else if (branches.get(level).isILPBranching()) {
					...
					if (!n.equals(node) && n.getId() < node.getId()) {
						...
						/*else if(!node.getSimpleILPLabel().isEmpty()){
							Set<OWLSubClassOfAxiom> disjointAx = new HashSet<>();
							df.getOWLDisjointUnionAxiom(owlClass, classExpressions, annotations)
						}*/else {
				...
				///
				else if (branches.get(level).isMergeBranching()) {
					System.err.println(
				...
				else if (branches.get(level).isDisjunctionBranching()){
					System.err.println("disjunction branching "+ branches.get(level).getNode().getId() +" curr node "+ cg.getCurrNode().getId());
					if (branches.get(level).hasNextOption()) {
						...
						//System.out.println("branchClashSet before "+ branches.get(level).getClashSet().getbpList());
						DependencySet newClashSet = DependencySet.create(clashSet);
						/*for(Integer bp : clashSet.getbpList()) {
							if(branches.get(bp).isInsideILP()) {
								newClashSet.removeLevel(bp);
							}
						}*/
						branches.get(level).setClashSet(DependencySet.plus(DependencySet.create(newClashSet),
								DependencySet.create(branchClashSet)));
						//System.out.println("branchClashSet after "+ branches.get(level).getClashSet().getbpList());
					...
					//	System.err.println(" curr node "+ cg.getCurrNode().getId());
				...
				clashSet.removeLevel(level);
				// System.out.println("clashSet after here " + clashSet.getbpList());
				if (!clashHandler(clashSet, node))
					return false;
	...
	private boolean handleClashInsideILPBranching(DependencySet clashSet, Node node, Map<Integer, BranchHandler> insideILPBranches, OWLObjectPropertyExpression role) {
		...
		Set<OWLSubClassOfAxiom> disjointness = new HashSet<>(); 
		if(insideILPBranches.size() > 1) {
		//	List<OWLClassExpression> disjuncts = new ArrayList<>();
			Set<OWLClassExpression> disjuncts = new HashSet<>();
			System.out.println("insideILPBranches size "+insideILPBranches.size());
			for(Entry<Integer, BranchHandler> en : insideILPBranches.entrySet()) {
				System.out.println("en "+en);
				Integer bl = en.getKey();
				BranchHandler bh = en.getValue();
				Node n = bh.getNode();
				System.out.println("node "+n +"bl "+bl );
				if(bh.hasNextOption()) {
					OWLClassExpression dj = n.getInsideILPDisjunct(bl);
					System.out.println("n.getInsideILPDisjunct().size() "+n.getInsideILPDisjunct().size() +" dj "+dj);
					
					
					if(!disjuncts.contains(dj)) {
						disjuncts.add(dj);
						
					}
				}
				else {
					return this.clashHandler(clashSet, node);
				}
			}
			if(disjuncts.size() == 1) {
				disjointness.add(df.getOWLSubClassOfAxiom(df.getOWLThing(), disjuncts.iterator().next().getComplementNNF()));
			}
			else {
			Set<OWLClassExpression> djCNNF = new HashSet<>();
			disjuncts.stream().forEach(dj -> djCNNF.add(dj.getComplementNNF()));
			
			disjointness.add(df.getOWLSubClassOfAxiom(df.getOWLThing(), df.getOWLObjectUnionOf(djCNNF)));
			}
			/*if(disjuncts.size() == 2) {
				OWLClassExpression ce1 = disjuncts.get(0);
				OWLClassExpression ce2 = disjuncts.get(1);
				disjointness.add(df.getOWLSubClassOfAxiom(df.getOWLThing(), df.getOWLObjectUnionOf(ce1.getComplementNNF(), ce2.getComplementNNF())));
				//disjointness = new HashSet<>(df.getOWLDisjointClassesAxiom(disjuncts).asOWLSubClassOfAxioms());
				
			}
			else {
				System.err.println("disjuncts.size() > 2");
			}
*/		}
		else if(insideILPBranches.size() == 1) {
			Entry<Integer, BranchHandler> en = insideILPBranches.entrySet().iterator().next();
			Integer bl = en.getKey();
			BranchHandler bh = en.getValue();
			Node n = bh.getNode();
			if(bh.hasNextOption()) {
				OWLClassExpression dj = n.getInsideILPDisjunct(bl);
				disjointness.add(df.getOWLSubClassOfAxiom(df.getOWLThing(), dj.getComplementNNF()));
		...
		int level = clashSet.getMax();
		BranchHandler bhILP = branches.get(level);
		System.out.println("n.getOutgoingEdges.size() "+bhILP.getNode().getOutgoingEdges().size());
		restore(level, true, false, false);
		System.out.println("after n.getOutgoingEdges.size() "+bhILP.getNode().getOutgoingEdges().size());
		return this.callILP(clashSet, bhILP.getNode(), bhILP.entries, bhILP.ds, disjointness, bhILP, role);
	}
	...
	public DependencySet getClashSet(Node n, OWLClassExpression ce, OWLClassExpression ceNNF) {
		 System.err.println("clash set exp "+ ce +" ceNNF "+ ceNNF );
...
*/
			...
			for (OWLClassExpression cn : sb.getSubClass().asConjunctSet()) {
				if (!classes.contains(cn)) {
					...
					for(ConceptNDepSet cnds : n.getnLabel().getCndList().getCdSet()) {
						if(cnds.getCe().equals(cn)) {
							flag = true;
							// System.err.println(" absorbRule2 cnds "+cnds.getCe() + " " +cnds.getDs().getbpList());
							dep.add(DependencySet.create(cnds.getDs()));
						}
					}
					/*n.getnLabel().getCndList().getCdSet().stream().filter(cnds -> cnds.getCe().equals(cn))
							.forEach(cnds -> dep.add(DependencySet.create(cnds.getDs())));*/
				}
			}
			if (flag) {
				OWLClassExpression c = sb.getSuperClass();
				// System.err.println(dep.getbpList() +" absorbRule2 "+c);
				if (c.isOWLNothing()) {
	...
	//	System.err.println("after incCurLevel :"+ getCurLevel());
	}
	public BranchHandler createInsideILPBranch(Node n, OWLObjectPropertyExpression role, OWLObjectUnionOf objUn, ConceptNDepSet cn, DependencySet ds) {
	//	System.err.println("level " +getCurLevel() + " node: "+ n.getId() + " objUn "+objUn);
		DependencySet newDs = DependencySet.plus(DependencySet.create(ds), DependencySet.create(getCurLevel()));
		BranchHandler bh = new BranchHandler(objUn, cn, newDs, true, role);
		this.branches.put(getCurLevel(), bh);
		save(n);
		incCurLevel();
		return bh;
	}
	/*public int getILPCurLevel() {
		return ILPBranchingPoint;
	}

	public void setILPCurLevel(int level) {
		ILPBranchingPoint = level;
	}

	public void incILPCurLevel() {
		++ILPBranchingPoint;
	}
*/
	...
	public void addToDoEntry(Node n, OWLClassExpression c, ConceptNDepSet cnds) {
		// System.err.println("add entry n "+ n.getId() +" "+ c +" ds "+ cnds.getDs().getbpList());
		if (c instanceof OWLObjectIntersectionOf)
	...
	public class BranchHandler {
		...
		private boolean insideILP = false;
		private boolean mergeBranching = false;
		private boolean disjunctionBranching = false;
		Set<Edge> outgoingEdges = new HashSet<>();
		Set<OWLSubClassOfAxiom> subsumption = new HashSet<>();
		Set<OWLSubClassOfAxiom> learnedSubsumption = new HashSet<>();
		ILPPreprocessor ilpPro = null;
		Map<OWLObjectPropertyExpression, ILPPreprocessor> ilpPros  = new HashMap<>();
		...
		protected BranchHandler(OWLObjectUnionOf objUn, ConceptNDepSet cnds, DependencySet ds, Node n) {
			...
			this.n = n;
			this.cnds = cnds;
			this.disjunctionBranching = true;
		}
		public void setIlpPro(ILPPreprocessor ilpPro) {
			this.ilpPro = ilpPro;
			
		}
		public void addIlpPro(OWLObjectPropertyExpression role, ILPPreprocessor ilpPro) {
			this.ilpPros.put(role, ilpPro);
			
		}
		public ILPPreprocessor getIlpPro(OWLObjectPropertyExpression role) {
			return this.ilpPros.get(role);
			
		}
		
		protected BranchHandler(OWLObjectUnionOf objUn, ConceptNDepSet cnds, DependencySet ds, boolean insideILP, OWLObjectPropertyExpression role) {
			objUn.asDisjunctSet().stream().forEach(ce -> applicableOrEntries.add(ce));
			this.size = applicableOrEntries.size();
			this.branchIndex = 0;
			this.objUn = objUn;
		...
		public boolean isMergeBranching() {
			return mergeBranching;
		}
		...
		protected void setDisjunctTaken(OWLClassExpression ce) {
			this.djTaken.add(ce);
			++branchIndex;
		}
		...
		public Set<OWLSubClassOfAxiom> getLearnedSubsumption() {
			return learnedSubsumption;
		}

		public void addLearnedSubsumption(OWLSubClassOfAxiom learnedSub) {
			this.learnedSubsumption.add(learnedSub);
		}
		...
		public boolean isInsideILP() {
			return insideILP;
		}
		public boolean isDisjunctionBranching() {
			return disjunctionBranching;
		}
	...
	public BranchHandler createInsideILPBranch(Node currNode2, OWLObjectUnionOf superClass,
			ConceptNDepSet conceptNDepSet, DependencySet create) {
		// TODO Auto-generated method stub 
		// created for ILPPreprocessor20July.java
		return null;
	}
	...
	private boolean canMergeNom(Node from, Node to) {
	//	System.err.println("can merge");
		if (from.getDisjointNodes().contains(to)) {
			return false;
		}
		for (OWLClassExpression c : from.getLabel()) {
		  ...
		  if(config.isSHOIQ() && to.isNominalNode()) { 
			  Set<ConceptNDepSet> maxCards = new HashSet<>();
		   maxCards.addAll(from.getnLabel().getCndList().getCdSet().stream() .filter(cnd -> cnd.getCe() instanceof OWLObjectMaxCardinality).collect(Collectors.toSet()));
		  maxCards.addAll(to.getnLabel().getCndList().getCdSet().stream() .filter(cnd -> cnd.getCe() instanceof OWLObjectMaxCardinality).collect(Collectors.toSet())); 
			  if(!checkAtMost(from, to, maxCards)) { 
				  return false; 
			  } 
		  }
