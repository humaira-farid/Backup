...
public class CplexModelGenerator8 {
	//IloCplex cplexModel;
	...
	public CplexModelGenerator8(ILPPreprocessor ilpPr, 
			Map<OWLClassExpression, Set<OWLClassExpression>> conceptSubsumersMap,
	...
	public ILPSolution solve(RMPModel rmpModel, PPModel ppModel) throws IloException{
		...
		/// Generating new columns ///
			...
			if( relaxed_opt < M ){
				...
				System.out.println("non zero cardinality " + sumh);
				if(nonInteger) {
				...
				/*	for ( int i = 0; i < x.getSize(); i++ ) {
					...
					return applyBranchAndPrice(-1, rmpModel, ppModel, rmpCplex, ppCplex, x, h,subsets, b, r, sr, rmpModel.Constraint);
					/////
					///
	 ...
	 * @throws IloException
	 */
	private ILPSolution applyBranchAndPrice(int start, RMPModel rmpModel, PPModel ppModel, IloCplex rmpCplex, IloCplex ppCplex, IloNumVarArray x1, IloNumVarArray h1,
		...
		//	System.out.println("cardinality second :"+ cardinality);
			...
			/*	for ( int i1 = 0; i1 < x1.getSize(); i1++ ) {
			 ...
			 returnSolution = applyBranchAndPrice2(start, rmpModel, ppModel, ppCplex, rmpCplex, x, h, subsets, b, r1, sr, Constraint1, part2, 2);
				
			 if(returnSolution.solved)
				return returnSolution; 
				
			 else
				 returnSolution = applyBranchAndPrice2(start, rmpModel, ppModel, ppCplex, rmpCplex, x, h, subsets, b, r1, sr, Constraint1, part1, 1);
			 if(returnSolution.solved)
					return returnSolution; 
	...
	private ILPSolution applyBranchAndPrice2(int start, RMPModel rmpModel, PPModel ppModel, IloCplex rmpCplex, IloCplex ppCplex, IloNumVarArray x1, IloNumVarArray h1,
			ArrayList<SubSet> subsets1, IloNumVar[] b, IloNumVar[] r1, IloNumVar[] sr, IloRange[] Constraint1, double part22, int m2) throws IloException {
		...
		/*for(int i = 0; i < size; i++) {
			...
			xvalues.put(x.getElement(i), rmpCplex.getValue(x.getElement(i)));
		}*/
	//	int i = -1;
	//	System.out.println("xvalues.keySet() :"+ xvalues.keySet().size());
		/*for ( IloNumVar xv: xvalues.keySet()) {
			//i++;
			double cardinality = xvalues.get(xv);
		  
			System.out.println("cardinality sec :"+ cardinality);
		}*/
		if(start>=size) {
					...
					if( relaxed_opt < M ){
						...
						if(nonInteger) {
							...
							//infeasible = true;
								...
								return applyBranchAndPrice(start, rmpModel, ppModel, rmpCplex, ppCplex, x, h, subsets, b, r, sr, Constraint);
